...
class perfect_defence:
        ...
        while not(new_dir==last_dir):
            ...
            for i in xrange(len(pirates)):
                ...
                if(dir_list[i][1]):
                    ...
                    if(len(possible_dir)+self.add_collision_dir(game,pirates[i])!=5):
                        flag = True
                    ...
                    if(len(dir_list[i][0])>1 and dir_list[i][0][1] in possible_dir):
                        ...
                        emergency = False
                    if(emergency):
                        if(can_cloak and game.is_passable(defence.ForwardMoving(pirates[i],dir_list[i][0][0]))):
                            can_cloak=False
                            flag = True
                            temp.append(dir_list[i][0][0])
    ...
    def add_collision_dir(self,game,pirate):
        collison = int(0)
        directionList=['n','e','s','w','-']
        loc = pirate.location
        for d in directionList:
            if not(game.is_passable(defence.ForwardMoving(pirate,d))):
                collison+=1
        return collison
