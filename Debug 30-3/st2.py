...
class st2:
    ...
    def Move(self,game, pirates):
        ...
        for i in xrange(len(pirates)):
            if(len(islands)>0):
                ...
                if direction[0]=='-':
                    game.debug("Possible Dir: "+str(possible_dir))
                if(direction[0] in possible_dir):
                    calc_dir = direction[0]
                elif(len(direction)>1 and direction[1] in possible_dir):
                    calc_dir = direction[1]
                elif(len(possible_dir)>0):
                    calc_dir = possible_dir[0]
                    if(self.oppdir[last_dir[pirate.id]]==calc_dir):
                        tmp= possible_dir.pop(0)
                        possible_dir.append(tmp)
                        calc_dir = possible_dir[0]
                
                game.set_sail(pirate,calc_dir)
                last_dir[pirate.id] = calc_dir
                ##else:
                    ##pirate.cloak()
    ...
    def Sail_Default(self,game,pirate):
        ...
        for p in pirates:
            dist.append(game.distance(pirate,p))
        index = dist.index(min(dist))
        if not(dist==0):
            direction = game.get_directions(pirate,pirates[index])
            possible_dir = self.defence.CheckIfInDanger(game,pirate)
            if(direction[0] in possible_dir):
